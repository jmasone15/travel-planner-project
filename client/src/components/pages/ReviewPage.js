    ...
    <div className="bgThis">
      ...
      >
        <form>
          ...
          <input
            type="text"
            placeholder="My Super Amazing Trip!"
            value={props.tripTripName}
            onChange={(e) => props.setTripName(e.target.value)}
          />
          <button
            className="btn mt-2 p-2 shadow"
            style={{ backgroundColor: "#Edd769" }}
            onClick={(e) => reviewTrip(e)}
          >
            Review Trip
          </button>
        </form>
      ...
      {showTripData ? (
        <div
          ...
          style={{
            width: "500px",
            marginTop: "50px",
            backgroundColor: "#E0e1cc",
          }}
        >
          {props.destinationPicture ? (
            <img
              src={`https://maps.googleapis.com/maps/api/place/photo?photoreference=${props.destinationPicture}&sensor=false&maxheight=250&maxwidth=250&key=AIzaSyCoiYtN7Xjb7P4JIpWRtlMiL9uQirs_icI`}
            />
          ) : null}
          <h3>
            <b>Your Trip to:</b> {props.tripDestination}
          </h3>
          <br />
          <p>
            <b>Trip name:</b> {props.tripTripName}
          </p>
          <p>
            <b>Budget:</b> {props.tripBudget}
          </p>
          <p>
            <b>Start Location: </b> {props.tripStartLocation}
          </p>
          <p>
            <b>Dates:</b> {getDateRange(props.tripDates)}
          </p>
          <br />
          <button
            className="btn btn-block btn-success mt-2 p-2 shadow"
            onClick={(e) => saveTravelPlans(e)}
          >
            Save Trip!
          </button>
          <button
